...
(function () {
  ...
  /* ---------- 14. Формы ----------
     Заявка уходит на свой REST-маршрут темы, а тот уже передаёт её в CRM
     «Мой класс». Ключ доступа остаётся на сервере и в браузер не попадает.
     Если сети нет или сервер ответил ошибкой — показываем телефон, чтобы
     посетитель не остался ни с чем. */

  var CFG = window.MAP_CFG || {};

  var collectUtm = function () {
  ...
  var setNote = function (form, text, isError) {
    var note = $('.form__msg', form);
    if (!note) {
      note = document.createElement('p');
      note.className = 'form__msg';
      form.appendChild(note);
    }
    note.textContent = text;
    note.classList.toggle('form__msg--error', !!isError);
  };
  ...
  var send = function (form) {
    ...
    Object.assign(data, collectUtm());
    data.page = location.href;

    if (btn) { btn.disabled = true; btn.textContent = strings.sending || 'Отправляем…'; }
    setNote(form, '');

    if (!CFG.restUrl) {
      // Тема отдаёт адрес маршрута через wp_localize_script; без него
      // отправлять некуда — честно говорим об этом.
      if (btn) { btn.disabled = false; btn.textContent = label; }
      setNote(form, strings.error || 'Не удалось отправить.', true);
      return;
    }
    ...
    fetch(CFG.restUrl + 'booking', {
      ...
      if (!res.ok || !res.body || !res.body.ok) {
        throw new Error((res.body && res.body.message) || 'error');
      }
      // Поля прячем, на их месте — подтверждение: заявка уже ушла, и
      // заполненная форма рядом с ней только сбивает с толку.
      form.classList.add('is-sent');

      var modal = form.closest ? form.closest('.modal') : null;

      if (modal) {
        // Окно закрывается само — искать крестик после отправки незачем.
        // В исходный вид форму вернёт closeModal.
      ...
      if (btn) { btn.disabled = false; btn.textContent = label; }
      var msg = (err && err.message && err.message !== 'error') ? err.message : null;
      setNote(form, msg || ((strings.error || 'Не удалось отправить.') +
        (CFG.phone ? ' ' + CFG.phone : '')), true);
    });
  };
