...
I['violin'] = '<svg viewBox="0 0 24 24"><path d="M12 8.8c-2.6 0-3.9 1.3-3.9 2.9 0 1.2 1.4 1.8 1.4 3 0 1.2-2 1.9-2 3.8 0 2 2 3.4 4.5 3.4s4.5-1.4 4.5-3.4c0-1.9-2-2.6-2-3.8 0-1.2 1.4-1.8 1.4-3 0-1.6-1.3-2.9-3.9-2.9z"/><path d="M12 8.8V4.4"/><path d="M12 4.4c0-1.2 1.7-1.5 2-.3.3 1.1-1 1.7-1.7 1.1"/><path d="M10.2 16.3v2.2M13.8 16.3v2.2"/></svg>'
I['trumpet'] = '<svg viewBox="0 0 24 24"><circle cx="2.8" cy="14.4" r="1.3"/><path d="M4.1 14.4h8.8"/><path d="M12.9 14.4c1.7-3.1 4.4-5 8-5.8v11.6c-3.6-.8-6.3-2.7-8-5.8z"/><path d="M6.9 14.4V9.8M9.3 14.4V9.8M11.7 14.4V9.8"/></svg>'
...
# ------------------------------------------------------------- направления
# Список стоит здесь, а не у своей секции: по нему считаются цифры на
# главной и на странице «О нас».
DIRECTIONS = [
...
NEWS = [
 dict(slug='camp', d='24', m='августа', y='2026', tag='Жизнь центра', img='g24.jpg',
      title='Фестиваль «Мосты искусства: от сердца к сердцу»',
      text='Педагоги центра работали в Творческой школе фонда ФОРТЕФОРМА в Махачкале — занятия, репетиции и концерты.',
      video=dict(src='https://kinescope.io/embed/upNVbibe76KF4niATiUAhq', typ='iframe',
                 page='https://kinescope.io/upNVbibe76KF4niATiUAhq', host='Kinescope',
                 poster='assets/img/gallery/g25.jpg',
                 title='«От сердца к сердцу»', sub='Видеохроника фестиваля')),
 dict(slug='concert', d='24', m='мая', y='2026', tag='Концерты', img='g21.jpg',
 ...
 dict(slug='master', d='17', m='апреля', y='2026', tag='Мастер-классы', img='g26.jpg',
      title='Открытый урок профессора А. Л. Маклыгина',
      text='Открытый урок по теории музыки и импровизации для учеников и родителей.'),
 dict(slug='open', d='06', m='апреля', y='2026', tag='Жизнь центра', img='g27.jpg',
      ...
      video=dict(src='assets/video/early-lesson.mp4', typ='file', vertical=True,
                 poster='assets/video/early-lesson-poster.jpg',
                 title='Фрагмент занятия', sub='Открытый урок раннего развития')),
 ...
 dict(slug='art', d='14', m='февраля', y='2026', tag='Жизнь центра', img='g28.jpg',
      ...
      photos=[('g30.jpg','Работа ученицы художественного отделения'),
              ('g31.jpg','Занятие у мольберта'),
              ('g33.jpg','Урок рисунка с педагогом'),
              ('g32.jpg','Готовая работа')]),
 dict(slug='nosina', d='24', m='января', y='2026', tag='Мастер-классы', img='g29.jpg',
      title='Мастер-класс профессора В. Б. Носиной',
      text='Творческая встреча с заслуженным деятелем искусств, профессором Верой Борисовной Носиной.'),
...
HERO_SLIDES = [('g20.jpg','Педагоги центра MusicArtPlus'),
               ('g11.jpg','Занятие по скрипке с ребёнком'),
               ('g23.jpg','Занятие в концертном зале центра'),
               ('g15.jpg','Открытый урок по духовым инструментам')]
...
# Третий столбец — что считать на сайте: в WordPress эти цифры не вписывают
# руками, а берут из числа карточек, иначе после каждого нового педагога
# пришлось бы править их вручную. Пустая строка — число задаётся руками.
FACTS = [(str(len(TEACHERS) + len(GUESTS)),'педагогов и приглашённых артистов','teachers_guests'),
         (str(len(DIRECTIONS)),'направлений обучения','directions'),
...
facts_html = ''.join('<div class="hero-fact"><div class="hero-fact__num">' + n + '</div>'
                     '<div class="hero-fact__label">' + l + '</div></div>' for n, l, _ in FACTS)
...
STATS = [(str(len(TEACHERS) + len(GUESTS)),'педагогов и приглашённых артистов','teachers_guests'),
         (str(len(DIRECTIONS)),'направлений обучения','directions'),
...
stats_html = ''.join('<div class="stat reveal" data-delay="' + str(i) + '"><b>' + n + '</b>'
                     '<span class="stat__l">' + l + '</span></div>' for i, (n, l, _) in enumerate(STATS))
...
# ================================================================ НОВОСТИ
FULL = {
 'camp': ['С 17 по 24 августа в столице Республики Дагестан, городе Махачкала, под эгидой фонда ФОРТЕФОРМА проходил Всероссийский фестиваль «Мосты искусства: от сердца к сердцу».',
          'В рамках фестиваля состоялась I Творческая школа «ФОРТЕФОРМА» для одарённых учащихся и студентов Республики Дагестан — в ней принимали активное участие наши педагоги.',
          'Как это было — в видеохронике фестиваля.'],
 ...
 'master': ['Александр Львович Маклыгин — доктор искусствоведения, профессор кафедры «Теория музыки» Московской консерватории им. П. И. Чайковского, председатель Общества теории музыки России.',
            'На открытом уроке речь шла о том, как слышать музыкальную форму и не бояться импровизации. Занятие было построено как диалог: ученики играли, задавали вопросы и спорили.',
            'Запись открытого урока доступна в разделе «Педагоги».'],
 ...
 'nosina': ['В нашей школе состоялась творческая встреча с заслуженным деятелем искусств, профессором Верой Борисовной Носиной.',
            'Вера Борисовна — блестящий музыкант и мудрый наставник: её энергия и глубина понимания музыки зажигают сердца учеников уже не один десяток лет.',
            'Мы от всей души благодарим нашу гостью и слушателей и с нетерпением ждём следующих встреч.'],
...
def article_video(v):
    """Видео внутри новости. Открывается в том же окне, что и ролики на главной."""
    if not v:
        return ''

    attrs = (' data-video="' + v['src'] + '" data-video-type="' + v['typ'] + '"')
    if v.get('page'):
        attrs += (' data-video-page="' + v['page'] + '"'
                  ' data-video-host="' + v.get('host', '') + '"')
    if v.get('vertical'):
        attrs += ' data-video-ratio="vertical"'

    return ('<figure class="article__video">'
            '<button class="video-card video-card--wide' + (' video-card--v' if v.get('vertical') else '') + '"'
            ' type="button"' + attrs + ' aria-label="Смотреть: ' + esc(v['title']) + '">'
            '<img src="' + v['poster'] + '" alt="" loading="lazy">'
            '<span class="video-card__play">' + I['play'] + '</span>'
            '<span class="video-card__cap"><b>' + v['title'] + '</b>'
            '<span>' + v['sub'] + '</span></span></button></figure>')


def article_photos(photos):
    """Фотографии к новости — сеткой под текстом."""
    if not photos:
        return ''

    cells = ''.join('<figure class="article__shot">'
                    '<img src="assets/img/gallery/' + img + '" alt="' + esc(alt) + '" loading="lazy">'
                    '</figure>' for img, alt in photos)

    return '<div class="article__gallery">' + cells + '</div>'
    ...
    body = (
      '<article class="section article">'
        '<div class="container container--narrow">'
          ...
          + article_video(n.get('video'))
          + article_photos(n.get('photos')) +
